...
- Object
  - Kubernetesオブジェクトは、Kubernetes上で永続的なエンティティ
  - Kubernetesはこれらのエンティティを使って、クラスタの状態を表す
    - どんなコンテナ化されたアプリが稼働しているか
    - また、それらがどのノード上で動いているか
    - それらのアプリケーションから利用可能なリソース
    - アプリケーションがどのようにふるまうかのポリシー
      - 再起動、耐障害性、アップグレードなど
  - 一度オブジェクトを作成すると、Kubernetesは常にそのオブジェクトが存在し続けるように動く
  - つまり、KubernetesObjectは**クラスターの望ましい状態**を表す
...
- 可用性
  ...
  - そのほか(Nearline,Coldline,Archive)
    ...
    - Regionalで99.90%
  - Multi-Regionの冗長性
    - 互いに100マイル離れた2つ以上のリージョンで構成される
    - 複数のリージョンで**非同期レプリケーション**される
...
- オブジェクトライフサイクル
  - ユースケース
    - 365日以上経過したオブジェクトのストレージクラスをColdlineにダウングレード
    - 2013/01/01より前に作成されたオブジェクトを削除
    - バージョニングが有効になっているバケット内で、最新3バージョンのみを維持
...
- HA構成
  - クラスタリングの事
  - HA構成されたCloudSQLはリージョンインスタンスともいう
  - プライマリインスタンスとスタンバイインスタンスは各ゾーンの永続ディスクへ**同期レプリケーション**する

- MySQLでは、外部のサーバをマスターとして、CloudSQLにリードレプリカを複製することができる。
  - PostgreSQLは非対応
...
- インスタンス
  - インスタンスの構成
    ...
    - **インスタンス作成後に構成変更できない**
  - ノード数
    - 選択したノード数により、そのインスタンスで利用可能なストレージ量が決まる
...
- コネクションドレイン
  - VMがインスタンスグループから削除されたときなどに、既存の進行中リクエストが完了する時間を確保する
  - 0 ~ 3600 秒 (0:無効 ~ 1時間) が指定できる
  - デフォルトは300秒に設定されてた
  - Terraformでinstance groupからVMを削除してみたら、5分くらいかかって、結局エラーになったけど、コンソールではちゃんと消えてた
...
- ロケーション
  ...
  - つまり、リージョン内のすべてのゾーンで冗長的に利用できるように管理される
...
- ポリシー
  ...
  - つまりPolicyはBindigで、Bindingはロールにメンバーを割り当てたもの
...
### Cloud VPN
- IPsecVPNを利用してVPCへ接続する
  - IPsec:Security Architecture for Internet Protocol 主にVPNで使われる暗号化プロトコル
- HA と Classicがある
- HA VPN
  - SLA 99.99%
- Classic VPN
